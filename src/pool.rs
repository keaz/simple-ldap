...
pub async fn  from(configuration: PoolConfiguration) -> LdapPool {
    ...
    for i in 1..= configuration.pool_size {
        let client = LdapClient::for_pool(configuration.url.as_str(), configuration.bind_dn.as_str(), configuration.bind_pw.as_str(), i).await;
...
impl LdapPool {
        ...
        while (not_used).len() == 0 {
            if let Some(max_pool) = self.configuration.max_size {
                ...
                if in_use_size == max_pool {
                    return Err(PoolError::MaxPoolReached("Max pool size reached".to_string()));
                }
                let client = LdapClient::for_pool(self.configuration.url.as_str(), self.configuration.bind_dn.as_str(), self.configuration.bind_pw.as_str(), in_use_size + 1).await;
    ...
    pub async fn put(&self, client: LdapClient) -> Result<(),PoolError> {
        ...
        if !is_in_used {
            return Err(PoolError::UnManagedClient("Client is not a managed client".to_string()));
        }
        let mut not_used = self.not_used.lock().await;
        not_used.push(client);
